...
angular.module('app', [])
    .controller('PainelCtrl', function($scope, $http) {
        ...
        $scope.playName = (prefixo, msg) => {
            let audio = new SpeechSynthesisUtterance();
            let voices = window.speechSynthesis.getVoices();
            //msg.voice = voices[10];
            audio.voiceURI = 'native';
            audio.volume = 1; // 0 to 1
            audio.rate = 1.9; // 0.1 to 10
            audio.pitch = 0.1; //0 to 2
            audio.text = prefixo + msg;
            audio.lang = 'pt-BR';
        ...
        $scope.chamar = function() {
            if (PainelWeb.started && $scope.senhas.length > 0) {
                ...
                // Se o campo nome do cliente estiver vazio, irá mostrar apenas a senha com a sigla.
				if (!senha.nomeCliente) {
                    PainelWeb.Speech.play(senha, speechParams());
}
				// função que chama o nome do Paciente se este campo for preenchido no momento que gerar a senha
				// Os campos entre aspas simples serão lidos em forma de string, altere conforme sua necessidade.
                else {
			PainelWeb.Alert.play($scope.config.alert,$scope.playName('Paciente ',senha.nomeCliente),$scope.playName('Comparecer ao Consultório ',senha.numeroLocal),$scope.playName(' ', 'Oftalmo')); 
				}
